...
def F_vibrational(x,qe1,qe2,qe3,qD):

    return R*(7*F_einstein(x,qe1)           # original set
              +5*F_einstein(x,qe2)          # of temperatures
              +3*F_einstein(x,qe3)          # and weights
              +9*F_debye(x,qD))
...
def rsd_lmfit(params,x,y):             # residuals function
    ...
    for j in range(len(x)):
        ...
        int_C.append(F_spectroscopy(x[j])
                     + F_vibrational(x[j],t_e1,t_e2,t_e3,t_D)
                     + A*x[j]*(F_spectroscopy(x[j])
                                + F_vibrational(x[j],t_e1,t_e2,t_e3,t_D))**2)
...
def main():
    ...
    params = Parameters()
    params.add('T_E1',value = 150.0,min = 0,max = 2000)  # 164.9 - results
    params.add('T_E2',value = 250.0,min = 0,max = 2000)  # 234.6   from reference
    params.add('T_E3',value = 850.0,min = 0,max = 2000)  # 861.0
    params.add('T_D',value = 150.0,min = 0,max = 2000)   # 130.3
    params.add('A',value = 1e-7,min = 0)                 # 4.14e-7
    ...
    T_E3_smooth = interpolate.spline(N,T3_einstein,x_spline)
    ...
    plot(N,T1_einstein,'go',label=r'$\theta_{E_1}$')
    hold('on')
    ...
    plot(N,T3_einstein,'ro',label=r'$\theta_{E_3}$')
    plot(x_spline,T_E3_smooth,'--')
